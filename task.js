...
//@ Развернуть массив в обратную сторону, не используя reverse()
/* Способ 1
let reversed = []
...
// Для того, чтобы решить данную задачу, необходимо вручную написать функцию ,которая будет имитировать метод flat()
...
function createFlat(array) {
   ...
   array.forEach((el) => {
      if (Array.isArray(el)) {
         ...
         //! метод concat приклеивает к данным, которые уже попали в result, новые элементы. Т.е. forEach проходит по каждому элементу один раз, на каждой итерации \\ ↑↑↑при условии, что мы мы обнаружили вложенный массив↑↑↑ \\ будем приклеивать данный элемент (el)
      } else {
         result.push(el)
      }
   })
...
// Внутри функции создаем переменную с пустым массивом, куда будем помещать результат
...
//? По сути, когда js доходит до вложенного массива, он обращает внимание на то, что у нас есть условие на этот счет и элементы вложенного массива клеет к текущему. Когда js выходит из вложенного массива, то дальше видит обычные элементы, который просто пушит. 
