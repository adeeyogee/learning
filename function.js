...
console.log(doSomething); // вызов функции
...
// второй критерий - отсуствие side эффектов при обращении к функции (выведение на страницу, выведение в консоль, запись логов и т.д.)
//если система меняется после запуска функции, значит есть каки-то побочные эффекты, который функция выполняет

/*function a(b) {    -----   Пример чистой функции
    return b*5
}
*/
/*
let c = 5
function a(b) {      ------     Пример функции, которая зависит от внешних обстоятельств (переменной с)
    return b*c
}
*/
...
bestFriend()
*/
// данная функция получила доуступ к переменной извне
